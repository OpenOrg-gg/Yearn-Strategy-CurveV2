...
contract Strategy is BaseStrategy {
   ...
   // ) public {
    ...
    ICurveFi public curve =
        ICurveFi(curvePool); // CurveV2 Pool. need this for buying more pool tokens.
    ...
    address public rewardsContract = 0x085A2054c51eA5c91dbF7f90d65e728c0f2A270f; // This is unique to each curve pool, this one is for 3Crypto
    uint256 public pid = 61; // this is unique to each pool
    uint256 public optimal = 0; // this is the optimal token to deposit back to our curve pool. 0 USDT, 1 WBTC, 2 WETH
        ...
        )
    {
        ...
        if (stakedTokens > 0 && claimableTokens > 0) {
            ...
            if (optimal == 0) {
                ...
                uint256 theTokenBalance = theToken.balanceOf(address(this));
                curve.add_liquidity([0, theTokenBalance, 0], 0);
            } else {
    ...
    function setOptimal(uint256 _optimal) external onlyAuthorized {
        if (_optimal == 0) {
            ...
            crvPath[0] = address(crv);
            crvPath[1] = address(weth);
            crvPath[2] = address(theToken);

            convexTokenPath = new address[](3);
            convexTokenPath[0] = address(convexToken);
            convexTokenPath[1] = address(weth);
            convexTokenPath[2] = address(theToken);
            optimal = 1;
        } else {
